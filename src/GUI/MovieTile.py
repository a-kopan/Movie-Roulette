...
class MovieTile(Frame):
    ...
    def download_image(master: Frame, url: str):
        if not url:
            return Image.new(mode="RGB", size=(666, 666), color="grey")
    ...
    def __init__(
        self, master: Frame, data: Movie, result_window=None, is_template=False
    ):
        ...
        self.text: Label = Label(
            master=master, text=self.display_info, font=("Times New Roman", 30)
        )
        ...
        self.title = Button(
            ...
            height=height,
            bg="grey",
            command=self.display_self_in_result,
        )
        ...
        self.remove = Button(
            ...
            bg="red",
            fg="white",
            command=self.remove_self_from_queue,
        )
