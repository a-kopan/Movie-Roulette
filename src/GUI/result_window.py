...
class ResultWindow(Toplevel):
    ...
    def add_templates(self, movies: list[MovieTile]):
        movie_f = Movie("Start")
        self.first_template = MovieTile(
            master=self.frame_saved_movies,
            data=movie_f,
            result_window=self,
            is_template=True,
        )
        movie_s = Movie("End")
        self.last_template = MovieTile(
            master=self.frame_saved_movies,
            data=movie_s,
            result_window=self,
            is_template=True,
    ...
    def remove_from_saved(self, tile=None):
        ...
        # Changes the button
        self.button_save_remove.config(
            bg="green", text="SAVE", command=self.add_to_saved
        )
        self.refresh_window()
    ...
    def add_to_saved(self):
        ...
        if not tile in self.saved:
            ...
            tile.saved = True
            self.button_save_remove.config(
                bg="red", text="REMOVE", command=self.remove_from_saved
            )
    ...
    def refresh_window(self):
        ...
        if self.current.saved:
            self.button_save_remove.config(
                bg="red", text="REMOVE", command=self.remove_from_saved, state=ACTIVE
            )
        else:
            self.button_save_remove.config(
                bg="green", text="SAVE", command=self.add_to_saved, state=ACTIVE
            )
    ...
    def __init__(self, movies: list[Movie]):
        ...
        # Movie poster canvas
        self.canvas_poster = Canvas(
            self, borderwidth=0, highlightthickness=0, bg=self.background_color
        )
        ...
        self.button_save_remove = Button(
            master=self,
            text="SAVE",
            bg="lightblue",
        ...
        self.label_text_info = Label(
            master=self.frame_text_information,
            font=("Times New Roman", 30),
        ...
        self.label_top_bar = Label(
            master=self.frame_top_bar,
            text="saved positions",
            font=("Times New Roman", 15),
            bg=self.frame_top_bar.cget("bg"),
        )
        ...
        self.frame_saved_movies.grid(
            column=2,
            row=0,
            sticky="nsew",
            pady=(self.frame_top_bar.cget("height") + 10, 10),
            padx=(10, 0),
        )
        ...
        self.label_warning = Label(
            master=self.frame_warning,
            text="Warning, if you switch to saved movie,\n the currently chosen one will be lost.",
            font=("Times New Roman", 15),
            bg="red",
        )  # , bg=self.frame_warning.cget('bg'))
        ...
        self.queue: list[MovieTile] = [
            MovieTile(master=self.frame_saved_movies, data=movie, result_window=self)
            for movie in movies
        ]
